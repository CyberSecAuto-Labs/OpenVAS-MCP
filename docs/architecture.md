...
2. **Integration tests** against a live Greenbone Community Edition stack verify real GMP operations (`integration.yml`) — runs on push and PRs targeting `develop` or `main`
...
6. **Vulnerability scan** — `grype` scans the SBOM and fails the release on `high` severity findings, gated by `scripts/grype_gate.py`, which filters CPython CVEs fixed only on release lines the image does not ship (see [ci.md](ci.md#interpreter-cves-and-the-release-line-filter))
